...
class TwitchBot:
    ...
    def _do_creative(self, nick: str, text: str) -> bool:
        """Handle a creative request (sing a song, write a poem, etc.).

        Generates 4-8 lines of creative content from the LLM and drip-feeds
        them into chat over several seconds, so it reads like a genuine
        performance instead of a wall of text. Returns True when a creative
        piece was produced and queued, False when the request was not
        creative or the model could not deliver.
        """
        import llm as llm_mod
        ...
        local = llm_mod._is_local(
            (self._opts.get("llm_base_url") or "").strip())
        system = chatai.CREATIVE_SYSTEM
        try:
            raw = llm_mod.chat_reply(
                system,
                chatai.creative_user_prompt(
                    text, snapshot, nick, persona=persona,
                    max_lines=8 if local else 15),
                self._opts,
                max_tokens=500)
        except Exception as exc:
            self._log(f"creative request error: {exc!r}")
            return False
        if not raw or chatai.declined(raw):
            self._log("creative request: model returned nothing")
            return False
        lines = chatai.clean_creative_lines(raw)
        if len(lines) < 2:
            self._log(f"creative request: not enough usable lines from: "
                      f"{raw[:120]!r}")
            return False
        self._say(self._fit(f"@{nick} ", lines[0]))
        rest = lines[1:]
        delay = max(2.0, self._beef_gap())
        for i, line in enumerate(rest, 1):
            wait = i * delay
            t = threading.Timer(wait, self._queue_say, args=(line,))
            t.daemon = True
            t.start()
        self._log(f"creative request from {nick}: {len(lines)} lines queued")
        return True
