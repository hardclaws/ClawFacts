...
def test_bad_config_is_reported_plainly():
    ...
    # And it must print the real line from that file, not a guessed key. The
    # field after _llm_options changes as new config sections are inserted.
    reported = bad[expected - 1]
    assert f"| {reported}" in text, (reported, text)
...
def main():
    ...
    # The log tee: the console is untouched, and the FILE gets a
    # timestamp on every line - half the log lines never had one, which
    # made 'what happened at 12:47' a chore to answer from a file.
    import io as _io
    import re as _re
    ...
    tee.write("[llm] hello log\n")
    tee.write("still the same line, ")
    tee.write("continued\n")
    tee.flush()
