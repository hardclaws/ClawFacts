...
#: Creative requests: "sing me a song", "make me a poem", "write a haiku",
#: "tell me a story", "rap for us", etc. These should produce multi-line
#: creative output delivered over several messages, not a single reply.
_CREATIVE = re.compile(
    r"\b(?:sing|song|poem|poetry|haiku|limerick|rap|rhyme|story|tale|"
    r"ballad|shanty|jingle|freestyle|verse|sonnet|ode|lyrics?)\b",
    re.IGNORECASE)
_CREATIVE_ASK = re.compile(
    r"\b(?:sing|write|make|compose|tell|give|drop|spit|recite|rap)\b"
    r".*\b(?:song|poem|poetry|haiku|limerick|rap|rhyme|story|tale|"
    r"ballad|shanty|jingle|freestyle|verse|sonnet|ode|lyrics?)\b",
    re.IGNORECASE)


def creative_request(text: str, names=()) -> bool:
    """True when the message asks the bot to produce creative multi-line
    content - a song, poem, story, rap, etc. Strips a leading address to
    the bot first, so 'doc, sing me a song' is caught the same as the bare
    'sing me a song'."""
    t = strip_address(text, names).strip()
    if not t:
        return False
    return bool(_CREATIVE_ASK.search(t))


CREATIVE_SYSTEM = (
    "You are performing a creative piece for a Twitch chat - a song, poem, "
    "rap, story or similar. Write it as a SEQUENCE of short lines, one per "
    "line, separated by single newlines. Each line must be under 240 "
    "characters. Write 4-8 lines total - enough to feel like a real "
    "performance, not so many that it floods chat. Stay in character as "
    "the bot's persona. Keep it fun, lively and appropriate for a trucking "
    "stream. No explicit content, no slurs, no links, no @mentions, no "
    "hashtags, no emoji except at most one per line. No command syntax. "
    "Return ONLY the lines of your piece, nothing else - no preamble, no "
    "explanation, no 'here's your song'. Just the lines."
)


def creative_user_prompt(text: str, lines: list, nick: str,
                          persona: str = "", max_lines: int = 10) -> str:
    """The prompt for a creative request. Gives the persona, the room
    context, and the specific ask."""
    out = []
    if persona:
        out.append(f"Your voice: {persona}")
        out.append("")
    out.append("Recent chat (for context):")
    for n, t in (lines or [])[-max_lines:]:
        out.append(f"{n}: {t}")
    out.append("")
    out.append(f"{nick} asked: {text}")
    out.append("")
    out.append("Write your piece now - one line per line, 4-8 lines, "
               "each under 240 characters. Return ONLY the lines.")
    return "\n".join(out)


def clean_creative_lines(raw: str) -> list:
    """Turn raw model output into a list of safe chat lines.

    Splits on newlines, cleans each line through clean_line(), and drops
    empties. Returns at most 8 lines - enough for a song or poem without
    flooding chat."""
    out = []
