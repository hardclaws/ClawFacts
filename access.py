...
class Helix:
    ...
    def top_streams(self, first: int = 100):
        """The biggest live channels right now, as (name, game, viewers).

        Twitch's own API guide: "Get Streams orders the list of streamers by
        the number of viewers they have." So page one *is* the top of Twitch
        at this moment - there is no separate "top channels" endpoint, and
        this is the honest way to get famous streamers rather than a list
        someone hardcoded two years ago.

        Returns None when the answer is unknown. [] would be a real answer;
        None means "could not look".
        """
        first = max(1, min(100, int(first)))
        try:
            data = self._fetch("/helix/streams", {"first": first})
        except urllib.error.HTTPError as exc:
            self.errors += 1
            print(f"[access] helix top streams HTTP {exc.code}", flush=True)
            return None
        except (urllib.error.URLError, OSError, ValueError) as exc:
            self.errors += 1
            print(f"[access] helix top streams failed: {exc!r}", flush=True)
            return None
        out = []
        for row in data.get("data") or []:
            ...
            try:
                ...
                viewers = 0
            out.append((name, row.get("game_name") or "", viewers))
        return out

    def channel_followers(self, broadcaster_id: str = "", pages: int = 3,
                          per_page: int = 100):
        """Who follows the channel, as (display name, followed_at).

        Needs the moderator:read:followers scope this bot already holds AND
        this token being a moderator of the channel. Twitch returns the total
        count either way, but only fills in the rows when both are true - so
        an empty list is not evidence that nobody follows the channel, it
        means "not allowed to look". None keeps those apart.
        """
        broadcaster_id = broadcaster_id or self.broadcaster_id
        if not broadcaster_id:
            return None
        per_page = max(1, min(100, int(per_page)))
        out, cursor = [], ""
        for _ in range(max(1, int(pages))):
            params = {"broadcaster_id": broadcaster_id, "first": per_page}
            if cursor:
                params["after"] = cursor
            try:
                data = self._fetch("/helix/channels/followers", params)
            except urllib.error.HTTPError as exc:
                self.errors += 1
                print(f"[access] helix follower list HTTP {exc.code} - is the "
                      f"'moderator:read:followers' scope on the token? Run "
                      f"'python3 bot.py --login' to re-authorise.", flush=True)
                return None
            except (urllib.error.URLError, OSError, ValueError) as exc:
                self.errors += 1
                print(f"[access] helix follower list failed: {exc!r}",
                      flush=True)
                return None
            for row in data.get("data") or []:
                name = row.get("user_name") or ""
                if name:
                    out.append((name, row.get("followed_at") or ""))
